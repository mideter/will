...
TEST_CASE("displace other session")
{
	...
	CHECK_FALSE(registry.bind_vessel(session1, will::domain::id::Vessel{7}));
	CHECK(registry.is_authenticated(session1));

	const auto displaced = registry.bind_vessel(session2, will::domain::id::Vessel{7});
	REQUIRE(displaced);
	CHECK(*displaced == session1);
	CHECK_FALSE(registry.is_authenticated(session1));
	CHECK(registry.is_authenticated(session2));
	CHECK(*registry.vessel_id(session2) == will::domain::id::Vessel{7});
}
...
TEST_CASE("same session rebind")
{
	...
	CHECK_FALSE(registry.bind_vessel(session, will::domain::id::Vessel{7}));
	CHECK_FALSE(registry.bind_vessel(session, will::domain::id::Vessel{7}));
	CHECK(registry.is_authenticated(session));
	CHECK(*registry.vessel_id(session) == will::domain::id::Vessel{7});
}
...
TEST_CASE("rebind different vessel clears old index")
{
	will::SessionRegistry registry;
	const will::SessionId session1 = registry.register_test_session()->id();
	const will::SessionId session2 = registry.register_test_session()->id();
	...
	CHECK_FALSE(registry.bind_vessel(session1, will::domain::id::Vessel{7}));
	CHECK_FALSE(registry.bind_vessel(session1, will::domain::id::Vessel{8}));

	CHECK_FALSE(registry.bind_vessel(session2, will::domain::id::Vessel{7}));
	CHECK(registry.is_authenticated(session1));
	CHECK(registry.is_authenticated(session2));
	CHECK(*registry.vessel_id(session1) == will::domain::id::Vessel{8});
	CHECK(*registry.vessel_id(session2) == will::domain::id::Vessel{7});
}
