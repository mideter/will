 ...
 * Owns session lifecycle, transport fan-out, and authenticated vessel bindings.
 * Enforces one active session per vessel.
 */
...
public:
	...
	void broadcast_except_vessel(domain::id::Vessel except_vessel_id, const v1::ServerEvent& event);

	std::optional<SessionId> session_id_for_vessel(domain::id::Vessel vessel_id) const;

	std::string_view peer_address(SessionId session_id) const;

	bool is_authenticated(SessionId session_id) const;

	std::optional<domain::id::Vessel> vessel_id(SessionId session_id) const;

	/** Binds vessel to session. Returns the displaced session id, if any. */
	std::optional<SessionId> bind_vessel(SessionId session_id, domain::id::Vessel vessel_id);
...
private:
	...
	void clear_vessel_binding(SessionId session_id);

	mutable std::mutex mutex_;
	std::unordered_map<std::uint64_t, std::shared_ptr<Session>> sessions_;
	std::unordered_map<std::uint64_t, SessionId> session_by_vessel_;
